...
const IMAGE_TYPES = ['png', 'webp', 'jpg', 'jpeg', 'svg']
...
const ASSET_SPECS: Record<MediaCategoria, {
  titulo: string
  formatos: string
  dimensao: string
  peso: string
  ...
  branding: {
    titulo: 'Logos e identidade',
    formatos: 'SVG, PNG ou WEBP',
    dimensao: 'Logo: 600x200 px | Favicon: 512x512 px',
    peso: 'até 300 KB',
    maxKb: 300,
    dica: 'Use fundo transparente sempre que possível.',
  },
  landing: {
    titulo: 'Landing Page / Hero',
    formatos: 'PNG ou WEBP',
    dimensao: 'Hero background: 1920x1080 px | Tigre PNG: 1200x1200 px',
    peso: 'até 800 KB',
    maxKb: 800,
    dica: 'Para o tigre do topo, prefira PNG transparente ou WEBP otimizado.',
  },
  dashboard: {
    titulo: 'Dashboard',
    formatos: 'WEBP ou PNG',
    dimensao: '1920x1080 px',
    peso: 'até 800 KB',
    maxKb: 800,
    dica: 'Use imagem escura e com pouco contraste para não atrapalhar a leitura.',
  },
  login: {
    ...
    peso: 'até 800 KB',
    maxKb: 800,
    dica: 'Imagem de impacto, mas com área livre para o formulário.',
  },
  mascote: {
    titulo: 'Mascote Tiger',
    formatos: 'PNG transparente ou WEBP',
    dimensao: '1200x1200 px',
    peso: 'até 1 MB',
    maxKb: 1024,
    dica: 'Ideal para Cyber Tiger, avatar, perfil e identidade visual.',
  },
  marketing: {
    titulo: 'Marketing / Banners',
    formatos: 'WEBP, PNG ou JPG',
    dimensao: '1920x1080 px ou 1080x1080 px',
    peso: 'até 1 MB',
    maxKb: 1024,
    dica: 'Use para campanhas, banners e chamadas promocionais.',
  },
  tema: {
    titulo: 'Temas Visuais',
    formatos: 'WEBP ou PNG',
    dimensao: '1920x1080 px',
    peso: 'até 1 MB',
    maxKb: 1024,
    dica: 'Use para temas sazonais: OAB, Elite, Black Friday etc.',
  },
  campanha: {
    titulo: 'Campanhas',
    formatos: 'WEBP, PNG, JPG ou MP4',
    dimensao: 'Imagem: 1920x1080 px | Vídeo: 1920x1080 px',
    peso: 'Imagem até 1 MB | Vídeo até 15 MB',
    maxKb: 15360,
    dica: 'Use para campanhas temporárias e banners especiais.',
  },
}

function isImageAsset(tipo: string) {
  return IMAGE_TYPES.includes(tipo)
}

function isVideoAsset(tipo: string) {
  return VIDEO_TYPES.includes(tipo)
}
...
function EmptyState({ msg, onAction }: { msg: string; onAction?: () => void }) {
  return (
    <div style={{ textAlign: 'center', padding: 48, color: '#555' }}>
      ...
      <div style={{ fontSize: 13, color: '#555', marginBottom: onAction ? 20 : 0 }}>
        {msg}
      </div>
      {onAction && (
        <button
...
function AssetCard({ asset, onClick }: { asset: MediaAsset; onClick: () => void }) {
  ...
  const isImage = isImageAsset(asset.tipo)
  const isVideo = isVideoAsset(asset.tipo)
  ...
  return (
    <div
      ...
      onMouseLeave={(e) => {
        ...
        e.currentTarget.style.borderColor = asset.ativo ? 'rgba(255,255,255,0.06)' : 'rgba(248,113,113,0.2)'
      }}
    >
      <div
        style={{
      ...
      >
        {!isImage && <span style={{ fontSize: 48 }}>{TIPO_ICONE[asset.tipo] || (isVideo ? '🎬' : '📦')}</span>}
...
function EditorAsset({ asset, adminId, onClose, onSaved }: DrawerProps) {
  ...
  const spec = ASSET_SPECS[form.categoria]
  ...
  const handleFileSelect = (f: File) => {
    const tamanhoKb = Math.round(f.size / 1024)
    const specAtual = ASSET_SPECS[form.categoria]

    if (specAtual && tamanhoKb > specAtual.maxKb) {
      setMsg(`⚠️ Arquivo acima do recomendado para ${specAtual.titulo}. Recomendado: ${specAtual.peso}.`)
    } else {
      setMsg('')
    }
      ...
      >
        ...
        >
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>
              {isNovo ? 'Upload de Asset' : 'Editar Asset'}
            </div>
            {!isNovo && asset && (
              <div style={{ fontSize: 11, color: '#555', fontFamily: 'monospace' }}>
                {asset.id.slice(0, 8)}… · {asset.storage_path}
              </div>
            )}
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 18 }}>
          {isNovo ? (
            <section>
              ...
              <div
                ...
                style={{
                  background: previewUrl
                    ? `url(${previewUrl}) center/contain no-repeat, repeating-conic-gradient(#1a1a1a 0% 25%, #0d0d0d 0% 50%) 50% / 12px 12px`
                    : 'rgba(255,255,255,0.03)',
              ...
              >
                {file ? (
                  <>
                    {!previewUrl && <div style={{ fontSize: 36 }}>📦</div>}
                    <div
                      style={{
                        fontSize: 13,
                        fontWeight: 600,
              ...
              <input
                ref={fileInputRef}
                type="file"
                accept=".png,.webp,.jpg,.jpeg,.svg,.mp4,.webm"
                style={{ display: 'none' }}
            ...
            <section>
              ...
              <div
                style={{
                  background: asset && isImageAsset(asset.tipo)
              ...
              >
                {asset && !isImageAsset(asset.tipo) && (
                  <span style={{ fontSize: 48 }}>{TIPO_ICONE[asset.tipo] || '📦'}</span>
                )}
              </div>
          ...
          <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              {label('Categoria')}
              {sel(form.categoria, (v) => set('categoria', v), MEDIA_CATEGORIAS.map((c) => [c, MEDIA_CATEGORIA_LABEL[c]]))}
            </div>
            <div>
              {label('Subcategoria (opcional)')}
              {inp(form.subcategoria, (v) => set('subcategoria', v), 'Ex: logos, banners...')}
            </div>
          </section>
          ...
          >
            <div style={{ fontSize: 12, fontWeight: 800, color: '#D4A843' }}>
              📐 Especificação recomendada — {spec.titulo}
            </div>

            <div style={{ fontSize: 11, color: '#aaa', lineHeight: 1.7 }}>
              <strong>Formatos:</strong> {spec.formatos}
              <br />
              <strong>Dimensão:</strong> {spec.dimensao}
              <br />
              <strong>Peso:</strong> {spec.peso}
              <br />
              <strong>Dica:</strong> {spec.dica}
            </div>
          </section>

          <div>
            {label('Nome')}
            {inp(form.nome, (v) => set('nome', v), 'Ex: Logo Principal')}
          </div>

          <div>
            {label('Alt Text (acessibilidade)')}
            {inp(form.alt_text, (v) => set('alt_text', v), 'Descrição curta')}
          </div>

          <div>
            {label('Descrição')}
            {txt(form.descricao, (v) => set('descricao', v), 2, 'Notas internas...')}
          </div>

          <div>
            {label('Tags (separadas por vírgula)')}
            {inp(form.tags, (v) => set('tags', v), 'logo, principal, dark')}
          </div>
          ...
          >
            <div
              style={{
                ...
                height: 22,
                borderRadius: 11,
                background: form.ativo ? '#34d399' : '#374151',
                position: 'relative',
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: '#666' }}>
                {form.ativo ? 'Visível e disponível para uso' : 'Oculto / desativado'}
              </div>
            </div>
          </div>
            ...
            <section>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ color: '#aaa' }}>{asset.tipo}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#666' }}>MIME</span>
                  <span style={{ color: '#aaa', fontFamily: 'monospace' }}>{asset.mime_type}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#666' }}>Tamanho</span>
                  <span style={{ color: '#aaa' }}>{asset.tamanho_kb}KB</span>
                </div>
                {asset.largura && asset.altura && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: '#666' }}>Dimensões</span>
                    <span style={{ color: '#aaa' }}>{asset.largura}×{asset.altura}</span>
                  </div>
                )}
...
export default function ModuloMediaLibrary({ adminId }: { adminId?: string }) {
  ...
  useEffect(() => {
    load(pagina, busca)
  }, [pagina, filtroCategoria, filtroAtivo, load, busca])
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12, color: '#555' }}>
              {total.toLocaleString()} asset(s) cadastrado(s)
            </div>
          </div>

          <div style={{ display: 'flex', gap: 8 }}>
            <button
              onClick={() => load(pagina, busca)}
              style={{
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <select
            ...
            onChange={(e) => setFiltroCategoria(e.target.value as MediaCategoria | '')}
            style={{
              background: '#1a1a1a',
              border: '1px solid rgba(255,255,255,0.1)',
              borderRadius: 8,
              padding: '9px 12px',
              color: filtroCategoria ? '#fff' : '#555',
              fontSize: 13,
              outline: 'none',
              colorScheme: 'dark',
              fontFamily: 'inherit',
          ...
          <select
            ...
            style={{
              ...
              borderRadius: 8,
              padding: '9px 12px',
              color: filtroAtivo !== 'todos' ? '#fff' : '#555',
              fontSize: 13,
              outline: 'none',
              colorScheme: 'dark',
              fontFamily: 'inherit',
      ...
      {loading ? (
        ...
        <>
          ...
          {totalPags > 1 && (
            ...
            >
              <div style={{ fontSize: 12, color: '#555' }}>
                Página {pagina + 1} de {totalPags} · {total.toLocaleString()} assets
              </div>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 6,
                    padding: '5px 12px',
                    color: pagina === totalPags - 1 ? '#444' : '#ccc',
                    fontSize: 12,
                    cursor: pagina === totalPags - 1 ? 'not-allowed' : 'pointer',
                  }}
                >
                  Próxima →
                </button>
                <button
                  onClick={() => setPagina(totalPags - 1)}
                  disabled={pagina === totalPags - 1}
                  style={{
                    background: '#1a1a1a',
                    border: '1px solid rgba(255,255,255,0.1)',
