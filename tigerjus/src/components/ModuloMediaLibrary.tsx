...
// ─── GUIA POR CATEGORIA / SUBCATEGORIA ────────────────────────────────────────

const CATEGORIA_DESCRICAO: Record<MediaCategoria, string> = {
  branding:  'Logos, favicons e elementos visuais da marca',
  landing:   'Hero, backgrounds e seções da página inicial',
  dashboard: 'Backgrounds, ícones e ilustrações da área logada',
  login:     'Backgrounds e ilustrações da tela de login/cadastro',
  mascote:   'Tigre e variações do mascote da marca',
  marketing: 'Banners, posts e materiais promocionais (redes sociais)',
  tema:      'Previews e ícones dos temas visuais',
  campanha:  'Banners de campanhas, ofertas e publicidade interna',
}

// Sugestões de subcategoria como chips clicáveis
const SUBCATEGORIAS_SUGERIDAS: Record<MediaCategoria, { value: string; label: string }[]> = {
  branding: [
    { value: 'logo-principal',     label: 'Logo Principal' },
    { value: 'logo-mobile',        label: 'Logo Mobile' },
    { value: 'favicon',            label: 'Favicon' },
    { value: 'logo-monocromatico', label: 'Logo Monocromático' },
  ],
  landing: [
    { value: 'hero-principal',     label: 'Hero Principal' },
    { value: 'background-landing', label: 'Background Landing' },
    { value: 'banner-topo',        label: 'Banner Topo' },
    { value: 'secao-features',     label: 'Seção Features' },
  ],
  dashboard: [
    { value: 'background-dashboard', label: 'Background Dashboard' },
    { value: 'mascote-loading',      label: 'Mascote Loading' },
    { value: 'card-vazio',           label: 'Card Vazio' },
  ],
  login: [
    { value: 'background-login', label: 'Background Login' },
    { value: 'imagem-cadastro',  label: 'Imagem Cadastro' },
  ],
  mascote: [
    { value: 'tigre-hero',        label: 'Tigre Hero' },
    { value: 'tigre-cabeca',      label: 'Tigre Cabeça' },
    { value: 'tigre-corpo',       label: 'Tigre Corpo Inteiro' },
    { value: 'tigre-alternativo', label: 'Tigre Alternativo' },
  ],
  marketing: [
    { value: 'banner-topo',   label: 'Banner Topo (1800×300)' },
    { value: 'feed-quadrado', label: 'Feed Quadrado (1080×1080)' },
    { value: 'feed-wide',     label: 'Wide (1920×1080)' },
    { value: 'stories',       label: 'Stories (1080×1920)' },
  ],
  tema: [
    { value: 'preview-tema', label: 'Preview Tema' },
    { value: 'icone-tema',   label: 'Ícone Tema' },
  ],
  campanha: [
    { value: 'cta-banner',           label: 'CTA Banner' },
    { value: 'publicidade-interna',  label: 'Publicidade Interna' },
    { value: 'oferta-especial',      label: 'Oferta Especial' },
    { value: 'campanha-promocional', label: 'Campanha Promocional' },
  ],
}

// Especificação técnica por subcategoria
const SUBCATEGORIA_GUIA: Record<string, {
  dimensao: string; formatos: string; peso: string; descricao: string
}> = {
  'logo-principal':       { dimensao: '512×512 px ou maior', formatos: 'SVG (preferencial) ou PNG transparente', peso: 'até 200 KB', descricao: 'Logo da empresa no header (desktop)' },
  'logo-mobile':          { dimensao: '256×256 px',          formatos: 'SVG ou PNG transparente',                peso: 'até 100 KB', descricao: 'Versão compacta do logo para telas pequenas' },
  'favicon':              { dimensao: '32×32 ou 64×64 px',   formatos: 'PNG ou SVG',                             peso: 'até 50 KB',  descricao: 'Ícone da aba do navegador e atalhos' },
  'logo-monocromatico':   { dimensao: '512×512 px',          formatos: 'SVG ou PNG (uma cor)',                   peso: 'até 150 KB', descricao: 'Versão em uma cor para fundos contrastantes' },

  'hero-principal':       { dimensao: '800×800 ou 1200×1200 px', formatos: 'PNG transparente ou WEBP', peso: 'até 500 KB (máx. 1 MB)', descricao: 'Imagem central do hero — geralmente o mascote' },
  'background-landing':   { dimensao: '1920×1080 px',            formatos: 'WEBP ou JPG',              peso: 'até 800 KB',             descricao: 'Fundo da landing (modo background)' },
  'banner-topo':          { dimensao: '1800×300 px',             formatos: 'WEBP, PNG ou JPG',         peso: 'até 1 MB',               descricao: 'Banner horizontal fino no topo da página' },
  'secao-features':       { dimensao: '600×400 px',              formatos: 'PNG ou WEBP',              peso: 'até 400 KB',             descricao: 'Ilustrações da seção de funcionalidades' },

  'background-dashboard': { dimensao: '1920×1080 px',        formatos: 'WEBP ou JPG',        peso: 'até 800 KB', descricao: 'Fundo do dashboard do usuário logado' },
  'mascote-loading':      { dimensao: '400×400 px',          formatos: 'PNG transparente',   peso: 'até 200 KB', descricao: 'Mascote para telas de loading e empty states' },
  'card-vazio':           { dimensao: '400×300 px',          formatos: 'PNG ou WEBP',        peso: 'até 200 KB', descricao: 'Ilustração para listas vazias' },

  'background-login':     { dimensao: '1920×1080 px',        formatos: 'WEBP ou JPG',        peso: 'até 800 KB', descricao: 'Fundo da tela de login/cadastro' },
  'imagem-cadastro':      { dimensao: '600×800 px',          formatos: 'PNG ou WEBP',        peso: 'até 500 KB', descricao: 'Ilustração lateral da tela de cadastro' },

  'tigre-hero':           { dimensao: '800×800 ou 1200×1200 px', formatos: 'PNG transparente ou WEBP', peso: 'até 500 KB (máx. 1 MB)', descricao: 'Mascote tigre — versão principal do hero' },
  'tigre-cabeca':         { dimensao: '400×400 px',              formatos: 'PNG transparente',         peso: 'até 200 KB',             descricao: 'Apenas a cabeça do tigre — avatar, ícones' },
  'tigre-corpo':          { dimensao: '600×900 px',              formatos: 'PNG transparente',         peso: 'até 400 KB',             descricao: 'Tigre de corpo inteiro — ilustrações grandes' },
  'tigre-alternativo':    { dimensao: '800×800 px',              formatos: 'PNG transparente',         peso: 'até 500 KB',             descricao: 'Variação do mascote (gestos diferentes, sazonal)' },

  'feed-quadrado':        { dimensao: '1080×1080 px',        formatos: 'WEBP, PNG ou JPG',   peso: 'até 1 MB',   descricao: 'Post quadrado para feed (Instagram, redes sociais)' },
  'feed-wide':            { dimensao: '1920×1080 px',        formatos: 'WEBP, PNG ou JPG',   peso: 'até 1 MB',   descricao: 'Banner horizontal (YouTube, capa de canal)' },
  'stories':              { dimensao: '1080×1920 px',        formatos: 'WEBP, PNG ou JPG',   peso: 'até 1 MB',   descricao: 'Story vertical (Instagram, TikTok, WhatsApp)' },

  'preview-tema':         { dimensao: '600×400 px',          formatos: 'PNG ou WEBP',        peso: 'até 300 KB', descricao: 'Miniatura do tema visual no seletor' },
  'icone-tema':           { dimensao: '128×128 px',          formatos: 'PNG ou SVG',         peso: 'até 50 KB',  descricao: 'Ícone identificador do tema' },

  'cta-banner':           { dimensao: '1200×600 px',         formatos: 'WEBP, PNG ou JPG',   peso: 'até 800 KB', descricao: 'Banner com call-to-action de campanha' },
  'publicidade-interna':  { dimensao: '1200×400 px',         formatos: 'WEBP, PNG ou JPG',   peso: 'até 800 KB', descricao: 'Banner promocional dentro da plataforma' },
  'oferta-especial':      { dimensao: '1200×600 px',         formatos: 'WEBP, PNG ou JPG',   peso: 'até 800 KB', descricao: 'Banner de oferta sazonal ou desconto' },
  'campanha-promocional': { dimensao: '1080×1080 px',        formatos: 'WEBP, PNG ou JPG',   peso: 'até 1 MB',   descricao: 'Material promocional para campanhas' },
}
...
function EditorAsset({asset, adminId, onClose, onSaved}: DrawerProps) {
  ...
  const sugestoes = SUBCATEGORIAS_SUGERIDAS[form.categoria] || []
  const guia = SUBCATEGORIA_GUIA[form.subcategoria]
  const corCategoria = CATEGORIA_COR[form.categoria] || '#888'
      ...
      <div style={{position:'fixed',top:0,right:0,bottom:0,zIndex:201,width:620,maxWidth:'100vw',background:'#111',borderLeft:'1px solid rgba(255,255,255,0.08)',overflowY:'auto',display:'flex',flexDirection:'column'}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:20,display:'flex',flexDirection:'column',gap:18}}>
          ...
          <section style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            ...
            <div>{label('Subcategoria (uso/aplicação)')}{inp(form.subcategoria, v => set('subcategoria', v), 'Ex: hero-principal, logo-mobile…')}</div>
          </section>

          {/* Descrição da categoria — orientação contextual */}
          <div style={{
            background:`${corCategoria}11`,
            border:`1px solid ${corCategoria}33`,
            borderRadius:8, padding:'10px 14px',
            display:'flex', alignItems:'center', gap:10,
            fontSize:12, color:'#aaa', lineHeight:1.5,
          }}>
            <span style={{fontSize:16}}>💡</span>
            <span><strong style={{color:corCategoria}}>{MEDIA_CATEGORIA_LABEL[form.categoria]}:</strong> {CATEGORIA_DESCRICAO[form.categoria]}</span>
          </div>

          {/* Chips de aplicações sugeridas — clicar preenche a subcategoria */}
          {sugestoes.length > 0 && (
            <div>
              {label('Aplicações sugeridas (clique para usar)')}
              <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                {sugestoes.map(s => {
                  const ativo = form.subcategoria === s.value
                  return (
                    <button key={s.value} type="button" onClick={() => set('subcategoria', s.value)}
                      style={{
                        background: ativo ? `${corCategoria}22` : 'rgba(255,255,255,0.03)',
                        border: `1px solid ${ativo ? corCategoria : 'rgba(255,255,255,0.08)'}`,
                        borderRadius: 100,
                        padding: '6px 12px',
                        fontSize: 11,
                        color: ativo ? corCategoria : '#888',
          ...
          {/* Card de guia técnica — aparece quando a subcategoria é conhecida */}
          {guia && (
            <div style={{
              ...
              <div style={{fontSize:10,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:'#D4A843'}}>
                📋 Guia rápido
              </div>
              <div style={{fontSize:12,color:'#ccc',lineHeight:1.7}}>
                <div><span style={{color:'#888'}}>📐 Dimensão:</span> <strong>{guia.dimensao}</strong></div>
                <div><span style={{color:'#888'}}>🎨 Formatos ideais:</span> {guia.formatos}</div>
                <div><span style={{color:'#888'}}>📦 Peso:</span> {guia.peso}</div>
                <div style={{marginTop:4,fontStyle:'italic',color:'#999'}}>💬 {guia.descricao}</div>
              </div>
            </div>
          )}
