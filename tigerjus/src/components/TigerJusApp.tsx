...
Legitimados: Presidente, Mesa do CN, PGR, OAB, partidos políticos, etc.`,
      ...
      {show && (
        <div style={{position:'absolute',bottom:'calc(100% + 8px)',left:'50%',transform:'translateX(-50%)',background:'#1a1a1a',border:'1px solid rgba(212,168,67,0.25)',borderRadius:12,padding:16,width:280,zIndex:100,boxShadow:'0 8px 32px rgba(0,0,0,0.6)'}}>
          ...
          <div style={{marginTop:10,fontSize:11,color:'var(--text-dim)'}}>
            Faltam <strong style={{color:'var(--gold)'}}>{(xpNext - xp).toLocaleString()} XP</strong> para o próximo nível
          </div>
          <div style={{marginTop:8,fontSize:11,color:'var(--text-muted)'}}>
            💡 Questão certa = +100 XP · Simulado = +500 XP · Login diário = +50 XP
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{padding:'32px 40px',flex:1,overflowY:'auto'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12,marginBottom:24}}>
        <div>
          <h1 style={{fontFamily:'var(--font-display)',fontSize:32,fontWeight:900,marginBottom:8}}>
            Olá, {profile?.nome?.split(' ')[0] || levelName}! 🔥
          </h1>
      ...
      <div style={{background:'linear-gradient(135deg,rgba(212,168,67,0.12),rgba(232,98,26,0.06))',border:'1px solid rgba(212,168,67,0.2)',borderRadius:20,padding:32,marginBottom:24,position:'relative',overflow:'hidden'}}>
        ...
        <div style={{fontFamily:'var(--font-display)',fontSize:32,fontWeight:900,marginBottom:4}}>
          <XPTooltip xp={xp} levelName={levelName} />
        </div>
        <div style={{fontSize:14,color:'var(--text-muted)',marginBottom:20}}>
          {xp < xpNext ? `${(xpNext - xp).toLocaleString()} XP para o próximo nível 🏆` : 'Nível máximo atingido! 👑'}
        </div>
      ...
      <div style={{background:'linear-gradient(135deg,rgba(58,143,232,0.08),rgba(212,168,67,0.06))',border:'1px solid rgba(58,143,232,0.2)',borderRadius:16,padding:24,marginBottom:28,cursor:'pointer'}}
        ...
        <div style={{fontSize:13,color:'var(--text-muted)'}}>Temas com maior probabilidade de cair na próxima OAB. Disponível no plano premium.</div>
      </div>
        ...
        <div style={{maxWidth:600,background:'var(--gray)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:20,padding:36}}>
          <div style={{marginBottom:20}}>
            ...
            <select value={disciplina} onChange={e => setDisciplina(e.target.value)}
              style={{width:'100%',background:'var(--gray-mid)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:10,padding:'12px 16px',color:'var(--white)',fontSize:14,fontFamily:'var(--font-body)'}}>
            ...
            <div style={{display:'flex',gap:10}}>
              {['','Fácil','Médio','Difícil'].map(d => (
                <button key={d} onClick={() => setDificuldade(d)}
                  style={{flex:1,padding:'10px',borderRadius:10,border:dificuldade===d?'1px solid rgba(212,168,67,0.4)':'1px solid rgba(255,255,255,0.08)',background:dificuldade===d?'rgba(212,168,67,0.1)':'transparent',color:dificuldade===d?'var(--gold)':'var(--text-muted)',fontSize:13,cursor:'pointer',fontFamily:'var(--font-body)'}}>
                  {d || 'Todas'}
                </button>
              ))}
            </div>
          ...
          <div style={{marginBottom:32}}>
            <label style={{fontSize:11,fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:'var(--text-muted)',display:'block',marginBottom:10}}>
              Quantidade: <span style={{color:'var(--gold)'}}>{quantidade} questões</span>
            </label>
            <input type="range" min={1} max={15} value={quantidade} onChange={e => setQuantidade(Number(e.target.value))}
    ...
    <div style={{padding:'32px 40px',flex:1}}>
      <div style={{maxWidth:720,margin:'0 auto'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
          ...
          <div style={{fontFamily:'var(--font-mono)',fontSize:20,fontWeight:700,color:time<20?'var(--danger)':'var(--gold)'}}>
    ...
    <div style={{padding:'32px 40px',flex:1,display:'flex',flexDirection:'column'}}>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:16}}>
        {chips.map(c => (
          <button key={c} onClick={() => send(c)}
            style={{background:'rgba(212,168,67,0.06)',border:'1px solid rgba(212,168,67,0.14)',borderRadius:100,padding:'6px 14px',fontSize:12,color:'var(--text-muted)',cursor:'pointer',fontFamily:'var(--font-body)',transition:'all 0.2s'}}
            onMouseEnter={e => { e.currentTarget.style.background='rgba(212,168,67,0.12)'; e.currentTarget.style.color='var(--gold)' }}
            onMouseLeave={e => { e.currentTarget.style.background='rgba(212,168,67,0.06)'; e.currentTarget.style.color='var(--text-muted)' }}>
            {c}
          </button>
        ))}
      </div>
      <div style={{flex:1,background:'var(--gray)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:20,overflow:'hidden',display:'flex',flexDirection:'column',minHeight:400}}>
        ...
        <div style={{display:'flex',gap:12,padding:16,background:'var(--gray-mid)',borderTop:'1px solid rgba(255,255,255,0.06)'}}>
          ...
          <button onClick={() => send()}
            style={{background:'linear-gradient(135deg,var(--gold),var(--orange))',border:'none',borderRadius:10,width:44,height:44,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:18,color:'var(--deep-black)',transition:'transform 0.2s'}}
...
function DisciplinesPage({ showPremium, profile }: any) {
  ...
  if (selected) {
    ...
    return (
      <div style={{padding:'32px 40px',flex:1}}>
        <button onClick={() => setSelected(null)} style={{display:'flex',alignItems:'center',gap:8,color:'var(--text-muted)',fontSize:13,border:'none',background:'none',cursor:'pointer',marginBottom:24,fontFamily:'var(--font-body)'}}>
...
function FlashCards({ disciplina }: { disciplina: string }) {
  const cards = [
    {front:'O que é Habeas Corpus?',back:'Remédio constitucional que protege a liberdade de locomoção. Cabe quando há coação ilegal ou ameaça ao direito de ir e vir (art. 5º, LXVIII, CF/88).'},
    {front:'O que é Mandado de Segurança?',back:'Protege direito líquido e certo não amparado por HC ou HD, quando o responsável for autoridade pública ou agente de pessoa jurídica no exercício de atribuições do Poder Público (art. 5º, LXIX, CF/88).'},
    {front:'O que são Cláusulas Pétreas?',back:'Limitações materiais ao poder de reforma constitucional. Não podem ser abolidas: forma federativa, voto direto/secreto/universal/periódico, separação dos poderes e direitos fundamentais (art. 60, §4º, CF/88).'},
      ...
      <div style={{perspective:1000,marginBottom:24,cursor:'pointer'}} onClick={() => setFlipped(f => !f)}>
        <div style={{position:'relative',height:240,transformStyle:'preserve-3d',transition:'transform 0.6s',transform:flipped?'rotateY(180deg)':'rotateY(0)'}}>
          <div style={{position:'absolute',inset:0,backfaceVisibility:'hidden',background:'var(--gray)',border:'1px solid rgba(212,168,67,0.2)',borderRadius:20,padding:32,display:'flex',alignItems:'center',justifyContent:'center',textAlign:'center'}}>
            <div>
              <div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:'var(--gold)',marginBottom:16}}>PERGUNTA</div>
              <div style={{fontSize:18,fontWeight:600,lineHeight:1.5}}>{cards[idx].front}</div>
              <div style={{marginTop:20,fontSize:12,color:'var(--text-muted)'}}>Clique para ver a resposta</div>
            </div>
          </div>
          <div style={{position:'absolute',inset:0,backfaceVisibility:'hidden',transform:'rotateY(180deg)',background:'linear-gradient(135deg,rgba(212,168,67,0.1),rgba(232,98,26,0.06))',border:'1px solid rgba(212,168,67,0.3)',borderRadius:20,padding:32,display:'flex',alignItems:'center',justifyContent:'center',textAlign:'center'}}>
            <div>
              <div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:'var(--gold)',marginBottom:16}}>RESPOSTA</div>
              <div style={{fontSize:15,lineHeight:1.7,color:'var(--text-muted)'}}>{cards[idx].back}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{display:'flex',gap:10,marginBottom:24,flexWrap:'wrap'}}>
        {(['geral','semanal','disciplina'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            style={{background:tab===t?'rgba(212,168,67,0.1)':'var(--gray)',border:tab===t?'1px solid rgba(212,168,67,0.3)':'1px solid rgba(255,255,255,0.06)',borderRadius:8,padding:'8px 16px',color:tab===t?'var(--gold)':'var(--text-muted)',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'var(--font-body)',textTransform:'capitalize'}}>
          ...
          <div key={r.pos} style={{display:'flex',alignItems:'center',gap:16,background:(r as any).me?'rgba(212,168,67,0.06)':'var(--gray)',border:(r as any).me?'1px solid rgba(212,168,67,0.25)':'1px solid rgba(255,255,255,0.05)',borderRadius:14,padding:'16px 20px'}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:14,fontWeight:600}}>
                {r.name}
                {(r as any).me && <span style={{marginLeft:8,fontSize:10,background:'rgba(212,168,67,0.15)',color:'var(--gold)',padding:'2px 8px',borderRadius:4}}>VOCÊ</span>}
              </div>
    ...
    <div style={{background:'var(--deep-black)',minHeight:'100vh'}}>
      ...
      <nav className="navbar">
        ...
        <div style={{display:'flex',gap:20,alignItems:'center'}}>
          {SIDEBAR.map(i => (
            <button key={i.key} onClick={() => setPage(i.key)}
              style={{color:page===i.key?'var(--gold)':'var(--text-muted)',fontSize:12,fontWeight:500,letterSpacing:1,textTransform:'uppercase',border:'none',background:'none',cursor:'pointer',fontFamily:'var(--font-body)',transition:'color 0.2s',borderBottom:page===i.key?'2px solid var(--gold)':'2px solid transparent',paddingBottom:2}}>
              {i.label}
            </button>
          ))}
        </div>
