...
/**
 * Banner do topo do Dashboard — carrossel de até 3 slides.
 * v2: respeita dashboard_banner_enabled e dashboard_banner_slide_X_enabled.
    ...
    <div style={{ marginBottom:20 }}>
      ...
      <div className="tj-dash-banner-frame"
        ...
        {slides.map((slide, i) => {
          ...
          const img = (
            <img className="tj-dash-banner-img" src={slide.url} alt={slide.alt}
              loading={i === 0 ? 'eager' : 'lazy'} style={{ opacity }}/>
          )
          return (
            <div key={i} className="tj-dash-banner-slide"
              style={{ opacity: isActive ? 1 : 0, pointerEvents: isActive ? 'auto' : 'none', zIndex: isActive ? 2 : 1 }}>
              {slide.link
                ? <a href={slide.link} target="_blank" rel="noopener noreferrer"
                     style={{ display:'block', width:'100%', height:'100%', lineHeight:0 }}>{img}</a>
                : img}
            </div>
          )
        })}
        <img className="tj-dash-banner-spacer" src={slides[Math.min(current, total - 1)].url} alt=""
          aria-hidden="true" style={{ display:'none', width:'100%', height:'auto', visibility:'hidden', position:'relative', zIndex:0 }}/>
        {total > 1 && (
          <>
